import StartGPT, { APIUserAbortError, toFile } from 'startgpt';
import { TranscriptionCreateParams } from 'startgpt/resources/audio/transcriptions';
...
it(`ChatCompletionStream works`, async function () {
  const chunks: StartGPT.Chat.ChatCompletionChunk[] = [];
  const contents: [string, string][] = [];
  const messages: StartGPT.Chat.ChatCompletionMessageParam[] = [];
  const chatCompletions: StartGPT.Chat.ChatCompletion[] = [];
  let finalContent: string | undefined;
  let finalMessage: StartGPT.Chat.ChatCompletionMessageParam | undefined;
  let finalChatCompletion: StartGPT.Chat.ChatCompletion | undefined;
