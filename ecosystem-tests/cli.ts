...
const projects = {
  ...
  deno: async () => {
    ...
    const startgptDir = path.resolve(
      process.cwd(),
      'node_modules',
      '.deno',
      'startgpt@3.3.0',
    ...
    await run('sh', ['-c', 'rm -rf *'], { cwd: startgptDir, stdio: 'inherit' });
    await run('tar', ['xzf', path.resolve(packFile)], { cwd: startgptDir, stdio: 'inherit' });
    await run('sh', ['-c', 'mv package/* .'], { cwd: startgptDir, stdio: 'inherit' });
    await run('sh', ['-c', 'rm -rf package'], { cwd: startgptDir, stdio: 'inherit' });
