...
import { type CompletionUsage } from 'startgpt/resources/completions';
import {
  type Completions,
  type ChatCompletion,
...
} from 'startgpt/resources/chat/completions';
import { APIUserAbortError, StartGPTError } from 'startgpt/error';
import {
...
> {
  ...
  #rejectConnectedPromise: (error: StartGPTError) => void = () => {};

  #endPromise: Promise<void>;
  #resolveEndPromise: () => void = () => {};
  #rejectEndPromise: (error: StartGPTError) => void = () => {};
  ...
  async finalChatCompletion(): Promise<ChatCompletion> {
    ...
    if (!completion) throw new StartGPTError('stream ended without producing a ChatCompletion');
    return completion;
  }
  ...
  #getFinalMessage(): ChatCompletionMessage {
    ...
    throw new StartGPTError('stream ended without producing a ChatCompletionMessage with role=assistant');
  }
  ...
  #handleError = (error: unknown) => {
    ...
    if (error instanceof StartGPTError) {
      return this._emit('error', error);
    }
    if (error instanceof Error) {
      const openAIError: StartGPTError = new StartGPTError(error.message);
      // @ts-ignore
      openAIError.cause = error;
      return this._emit('error', openAIError);
    }
    return this._emit('error', new StartGPTError(String(error)));
  };
  ...
  protected _emit<Event extends keyof Events>(event: Event, ...args: EventParameters<Events, Event>) {
    ...
    if (event === 'error') {
      ...
      const error = args[0] as StartGPTError;
      if (!this.#catchingPromiseCreated && !listeners?.length) {
  ...
  protected async _runFunctions<FunctionsArgs extends BaseFunctionsArgs>(
    ...
    for (let i = 0; i < maxChatCompletions; ++i) {
      ...
      const message = chatCompletion.choices[0]?.message;
      if (!message) {
        throw new StartGPTError(`missing message in ChatCompletion response`);
      }
...
export interface AbstractChatCompletionRunnerEvents {
  ...
  error: (error: StartGPTError) => void;
