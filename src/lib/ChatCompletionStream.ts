...
import { type ReadableStream } from 'startgpt/_shims/index';
  ...
  return {
    ...
    choices: choices.map(
      ({ message, finish_reason, index, logprobs, ...choiceRest }): ChatCompletion.Choice => {
        if (!finish_reason) throw new StartGPTError(`missing finish_reason for choice ${index}`);
        const { content = null, function_call, tool_calls, ...messageRest } = message;
        const role = message.role as 'assistant'; // this is what we expect; in theory it could be different which would make our types a slight lie but would be fine.
        if (!role) throw new StartGPTError(`missing role for choice ${index}`);
        if (function_call) {
          const { arguments: args, name } = function_call;
          if (args == null) throw new StartGPTError(`missing function_call.arguments for choice ${index}`);
          if (!name) throw new StartGPTError(`missing function_call.name for choice ${index}`);
          return {
            ...choiceRest,
