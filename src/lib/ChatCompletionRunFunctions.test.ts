...
import { StartGPTError } from 'startgpt/error';
...
} from 'startgpt/resources/beta/chat/completions';
import type { ChatCompletionMessageParam } from 'startgpt/resources/chat/completions';

import { type RequestInfo, type RequestInit } from 'startgpt/_shims/index';
...
function mockChatCompletionFetch() {
  ...
  function handleRequest(
    handler: (body: ChatCompletionFunctionRunnerParams<any[]>) => Promise<StartGPT.Chat.ChatCompletion>,
  ): Promise<void> {
...
function mockStreamingChatCompletionFetch() {
  ...
  function handleRequest(
    ...
    ) => AsyncIterable<StartGPT.Chat.ChatCompletionChunk>,
  ): Promise<void> {
    return handleRawRequest(async (req, init) => {
      const rawBody = init?.body;
      if (typeof rawBody !== 'string') throw new Error(`expected init.body to be a string`);
...
function* contentChoiceDeltas(
  content: string,
  {
    index = 0,
    role = 'assistant',
  }: { index?: number; role?: NonNullable<StartGPT.Chat.ChatCompletionChunk.Choice.Delta['role']> } = {},
): Iterable<StartGPT.Chat.Completions.ChatCompletionChunk.Choice> {
...
function* functionCallDeltas(
  args: string,
  {
    ...
    id?: string;
    index?: number;
    role?: NonNullable<StartGPT.Chat.ChatCompletionChunk.Choice.Delta['role']>;
  },
): Iterable<StartGPT.Chat.Completions.ChatCompletionChunk.Choice> {
...
class RunnerListener {
  ...
  readonly chatCompletions: StartGPT.Chat.ChatCompletion[] = [];
  readonly functionCalls: StartGPT.Chat.ChatCompletionMessage.FunctionCall[] = [];
  readonly functionCallResults: string[] = [];
  finalContent: string | null = null;
  finalMessage: ChatCompletionMessageParam | undefined;
  finalChatCompletion: StartGPT.Chat.ChatCompletion | undefined;
  finalFunctionCall: StartGPT.Chat.ChatCompletionMessage.FunctionCall | undefined;
  finalFunctionCallResult: string | undefined;
  totalUsage: StartGPT.CompletionUsage | undefined;
  error: StartGPTError | undefined;
  gotConnect = false;
  gotAbort = false;
  gotEnd = false;
  ...
  async sanityCheck({ error }: { error?: string } = {}) {
    ...
    if (this.chatCompletions.some((c) => c.usage)) {
      const totalUsage: StartGPT.CompletionUsage = {
        completion_tokens: 0,
        prompt_tokens: 0,
        total_tokens: 0,
      };
...
class StreamingRunnerListener {
  readonly eventChunks: StartGPT.Chat.ChatCompletionChunk[] = [];
  readonly eventContents: [string, string][] = [];
  readonly eventMessages: ChatCompletionMessageParam[] = [];
  readonly eventChatCompletions: StartGPT.Chat.ChatCompletion[] = [];
  readonly eventFunctionCalls: StartGPT.Chat.ChatCompletionMessage.FunctionCall[] = [];
...
function _typeTests() {
  ...
  startgpt.beta.chat.completions.runTools({
    messages: [
...
describe('resource completions', () => {
  describe('runTools with stream: false', () => {
    test('successful flow', async () => {
      ...
      const startgpt = new StartGPT({ apiKey: 'something1234', baseURL: 'http://127.0.0.1:4010', fetch });

      const runner = startgpt.beta.chat.completions.runTools({
  ...
  describe('runTools with stream: true', () => {
    test('successful flow', async () => {
      ...
      await Promise.all([
        handleRequest(async function* (request): AsyncIterable<StartGPT.Chat.ChatCompletionChunk> {
          expect(request.messages).toEqual([{ role: 'user', content: 'tell me what the weather is like' }]);
          yield {
            id: '1',
            choices: [
    ...
    test('flow with abort', async () => {
      ...
      await handleRequest(async function* (request): AsyncIterable<StartGPT.Chat.ChatCompletionChunk> {
        expect(request.messages).toEqual([{ role: 'user', content: 'tell me what the weather is like' }]);
        yield {
          id: '1',
          choices: [
  ...
  describe('stream', () => {
    test('successful flow', async () => {
      ...
      const runner = startgpt.beta.chat.completions.stream({
        stream: true,
        messages: [{ role: 'user', content: 'tell me what the weather is like' }],
        model: 'gpt-3.5-turbo',
      });
