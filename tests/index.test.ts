...
import { APIUserAbortError } from 'startgpt';
import { Headers } from 'startgpt/core';
...
describe('instantiate client', () => {
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new StartGPT({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    test('explicit option', () => {
      const client = new StartGPT({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['OPENAI_BASE_URL'] = 'https://example.com/from_env';
      const client = new StartGPT({ apiKey: 'My API Key' });
  ...
  test('maxRetries option is correctly set', () => {
    const client = new StartGPT({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new StartGPT({ apiKey: 'My API Key' });
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new StartGPT({ apiKey: 'My API Key', timeout: 10, fetch: testFetch });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new StartGPT({ apiKey: 'My API Key', fetch: testFetch });
