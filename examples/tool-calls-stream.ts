...
} from 'startgpt/resources/chat';

// gets API Key from environment variable OPENAI_API_KEY
const startgpt = new StartGPT();

const tools: StartGPT.Chat.Completions.ChatCompletionTool[] = [
  {
    type: 'function',
    function: {
      name: 'list',
...
async function callTool(tool_call: StartGPT.Chat.Completions.ChatCompletionMessageToolCall): Promise<any> {
  ...
  const reduce = (acc: any, delta: ChatCompletionChunk.Choice.Delta) => {
    ...
    for (const [key, value] of Object.entries(delta)) {
      if (acc[key] === undefined || acc[key] === null) {
        acc[key] = value;
        //  StartGPT.Chat.Completions.ChatCompletionMessageToolCall does not have a key, .index
