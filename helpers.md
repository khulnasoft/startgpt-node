...
```ts
startgpt.chat.completions.stream({ stream?: false, … }, options?): ChatCompletionStreamingRunner
```

`startgpt.chat.completions.stream()` returns a `ChatCompletionStreamingRunner`, which emits events, has an async
iterator, and exposes helper methods to accumulate chunks into a convenient shape and make it easy to reason
about the conversation.

Alternatively, you can use `startgpt.chat.completions.create({ stream: true, … })` which returns an async
...
```ts
startgpt.chat.completions.runTools({ stream: false, … }, options?): ChatCompletionRunner
startgpt.chat.completions.runTools({ stream: true, … }, options?): ChatCompletionStreamingRunner
```

`startgpt.chat.completions.runTools()` returns a Runner
...
The first event that is fired when the connection with the StartGPT API is established.
